...
> ほかのページにも対応させたい場合は `userscript/sanrio-autofill.user.js` の `@match` を追加してください。

## 使い方（インストール）
1. Chrome/Edge/Firefox に [Tampermonkey](https://www.tampermonkey.net/) をインストール。
2. Tampermonkey で「新規スクリプト」を開き、`userscript/sanrio-autofill.user.js` の内容を **そのまま貼り付けて保存**。
